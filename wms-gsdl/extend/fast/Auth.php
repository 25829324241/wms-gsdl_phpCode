...
class Auth
{
     ...
     */
    public function check($name, $uid, $relation = 'or', $mode = 'url' , $cid)
    {
        if (!$this->config['auth_on']) {
            return true;
        }
        // 获取用户需要验证的所有有效规则列表
        $rulelist = $this->getRuleList($uid,$cid);
     ...
     * @throws \think\exception\DbException
     */
    public function getGroups($uid,$cid)
    {
        static $groups = [];
        if (isset($groups[$uid])) {
            return $groups[$uid];
        }
        //write_log($cid,'getGroups_');
        // 执行查询
        if(!empty($cid)){
            $user_groups = Db::name($this->config['auth_group_access'])
                ->alias('aga')
                ->join('__' . strtoupper($this->config['auth_group']) . '__ ag', 'aga.group_id = ag.id and ag.cid=aga.cid', 'LEFT')
                ->field('aga.uid,aga.group_id,ag.id,ag.pid,ag.name,ag.rules,aga.cid,ag.cid')
                ->where("aga.uid='{$uid}' and ag.cid='{$cid}' and ag.status='normal'")
                ->select();
        }else{
            $user_groups = Db::name($this->config['auth_group_access'])
                ->alias('aga')
        ...
        //write_log(var_export($user_groups,true),'groups_');
     ...
     * @throws \think\exception\DbException
     */
    public function getRuleList($uid,$cid)
    {
        ...
        //读取用户组所有权限规则
        $this->rules = Db::name($this->config['auth_rule'])->where($where)->field('id,pid,condition,icon,name,title,ismenu,cid')->select();
    ...
    public function getRuleIds($uid,$cid)
    {
        //读取用户所属用户组
        $groups = $this->getGroups($uid,$cid);
        //write_log(var_export($groups,true),'groups_');
