...
class Auth extends \fast\Auth
{
    ...
    public function check($name, $uid = '', $relation = 'or', $mode = 'url',$cid='')
    {
        ...
        return parent::check($name, $uid, $relation, $mode,$cid);
    }
    ...
    public function getGroups($uid = null,$cid = null)
    {
        $uid = is_null($uid) ? $this->id : $uid;
        //渠道id
        $cid = $cid ? $cid : $this->cid;
        return parent::getGroups($uid,$cid);
    }

    public function getRuleList($uid = null,$cid = null)
    {
        $uid = is_null($uid) ? $this->id : $uid;
        //渠道id
        $cid = $cid ? $cid : $this->cid;
        return parent::getRuleList($uid,$cid);
    }
    ...
    public function getRuleIds($uid = null,$cid = null)
    {
        $uid = is_null($uid) ? $this->id : $uid;
        //渠道id
        $cid = $cid ? $cid : $this->cid;
        return parent::getRuleIds($uid,$cid);
    }
     ...
     * @return array
     */
    public function getGroupIds($uid = null,$cid = null)
    {
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = collection(\app\admin\model\AuthRule::where(['status'=>'normal','cid'=>$this->cid])
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache("__menu__")
            ->select())->toArray();
        write_log(var_export($ruleList,true),'ruleList_');
        $indexRuleList = \app\admin\model\AuthRule::where(['status'=>'normal','cid'=>$this->cid])
            ->where('ismenu', 0)
