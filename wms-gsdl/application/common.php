...
use think\Response;
use app\common\model\SysNumbering as SysNumberingModel;
...
if (!function_exists('write_log')) {
     ...
     * @param $name
     */
    function write_log($text,$name){
        file_put_contents ( APP_PATH .DS . "log" . DS .$name."log.txt", date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
    }
}
...
if (!function_exists('get_url_query')) {
     ...
     * @return string
     */
    function get_url_query($url, array $array_query, $flag = false)
    {
        $params = http_build_query($array_query);
        if (!$flag && !empty($params)) {
            $params = '?' . $params;
        }
        return $url . $params;
    }
...
if (!function_exists('folder_url')) {
     ...
     * @return string
     */
    function folder_url($str = '', $vars = '', array $param = [], $suffix = true, $domain = false)
    {
        $module_name = request()->module();
        $str = empty($str) ? $module_name : $module_name . '/' . $str;
        return get_url_query(url($str, $vars, $suffix, $domain), $param);
    }
}

if (!function_exists('go_curl')) {
    function go_curl($url, $type, $data = false, $header=array() ,&$err_msg = null, $timeout = 20, $cert_info = array())
    {
        ...
        if (!empty($cert_info['ca_file'])) {
            ...
            $option[CURLOPT_SSL_VERIFYHOST] = 0;//额外添加的
        }
        //头设置
        if(!empty($header)){
        ...
        if ($curl_no > 0) {
            if ($err_msg !== null) {
                ...
                write_log(var_export($err_msg,true),'curl_error_data_');
            }
        }
        return $response;
    }
...
//系统单号
if (!function_exists('get_sys_order_sn')) {

    function get_sys_order_sn($type_name =''){

        $order_sn ='';


        $rules = SysNumberingModel::where(['numbering_prefix'=>$type_name])->find();
        if(!empty($rules)){
            $order_sn .=$type_name;
            //年
            if($rules['year_switch']==1){
               // date('Y-m-d')
                $order_sn .=date('Y');
            }
            //月
            if($rules['month_switch']==1){
            ...
            $order_sn .=str_pad(($rules['num']+1),$rules['numbering_num'],"0",STR_PAD_LEFT);
            SysNumberingModel::where(['numbering_prefix'=>$type_name])->setInc('num','1');
        }

        if(empty($type_name) && empty($rules)){
...
if (!function_exists('get_order_sn')) {
    function get_order_sn($id = 0)
    {
        mt_srand($id);
        return date('md') . substr(implode(null, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8) . substr(mt_rand(), 4, 4) . rand(111, 999);
    }
}